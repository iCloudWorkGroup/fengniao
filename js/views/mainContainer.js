...
define(function(require) {
			...
			var MainContainer = Backbone.View.extend({
						...
						deleteBottom: function(recordPosi) {
							...
							recordIndex = binary.indexModelBinary(recordPosi, headItemRowList, 'top', 'height'); << << << < HEAD
								=== === =
						...
						addBottom: function(recordPosi) {
							...
							loadBottomPosi = loadBottomPosi || limitBottomPosi; === === = >>> >>> > feat(entrance / sheet / undoredu): complete undoredo feature
						...
						loadRegion: function(top, bottom) {
							var headItemRowList = headItemRows.models,
								...
								topIndex,
								bottomIndex, << << << < HEAD
							rowLoadBottom, === === = >>> >>> > feat(entrance / sheet / undoredu): complete undoredo feature
							height,
							i = 0;
							...
							bottom = bottom < cache.localRowPosi ? bottom : cache.localRowPosi; << << << < HEAD
								=== === =
								//需要保存准备值
								>>> >>> > feat(entrance / sheet / undoredu): complete undoredo feature
							if (isUnloadRows) {
								...
								loadRecorder.insertPosi(headItemRowList[topIndex].get('top'),
									...
									cache.rowRegionPosi); << << << < HEAD
								rowLoadBottom = bottom; === === = >>> >>> > feat(entrance / sheet / undoredu): complete undoredo feature
							}
							if (isUnloadCells) {
												...
												requestRows: function(top, bottom) {
													send.PackAjax({
														...
														data: JSON.stringify({
															...
															sheetId: '1',
															rowBegin: top,
															rowEnd: bottom >>> >>> > feat(entrance / sheet / undoredu): complete undoredo feature
														}),
